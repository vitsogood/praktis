        ...
        if ($result && $result -> num_rows > 0)
        {
            while ($requestData = $result -> fetch_assoc())
            {
                ...
                if(file_exists($arkDrawing))
                {
                    ...
                    $viewUrl = "drawingViewer('".$partId."',this);";
                    $arkDwg = "<i style='cursor:pointer; margin-left: 40% !important; font-size: 16px !important' id='centerDwgBtn' class='fa fa-photo w3-medium w3-text-purple drawingViewerClass' data-value=".$partId." onclick=\"".$viewUrl."\" title='Drawing'></i>";
                }
                else
			    {
				    $cusDwg = "<font color='black'>SUB</font>";
			    }


                // if(file_exists($cusDrawing))
			    // {
				// $ext = pathinfo($cusDrawing, PATHINFO_EXTENSION);
				// $url = "window.open('/Document Management System/Master Folder/MAIN_".$partId.".".$ext."','cc','left=50,screenX=700,screenY=20,resizable,scrollbars,status,width=1000,height=700')";
				// $viewUrl = "drawingViewer('".$partId."', '".$db->real_escape_string($sqlData)."', 'cus',this);";
				// $cusDwg = "<i style='cursor:pointer;' class='fa fa-photo w3-medium w3-text-purple' onclick=\"".$viewUrl."\" title='Drawing'></i>";
			    // }
                ...
                //     $dwg3DF = "<a href='/Document Management System/3D Files/3DF_".$partId.".".$ext."' download><i style='cursor:pointer;' class='fa fa-photo w3-medium w3-text-purple' title='Drawing'></i></a>";
                // }
                ...
                $nestedData[] = $cusDwg;
                // $nestedData[] = $dwg3DF;
                // $nestedData[] = $workInstruction;
                // $nestedData[] = $workInstructions;
                $data[] = $nestedData;
